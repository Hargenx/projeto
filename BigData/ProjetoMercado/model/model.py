...
class ModeloSupermercado:
    ...
    def criar_coluna_mes(self):
        self.df["Mês"] = self.df["Date"].apply(lambda x: str(x.year) + "-" + str(x.month))
    ...
    def calcular_total_custo(self, mes):
        df_filtrado = self.obter_dados_por_mes(mes)
        total_custo = df_filtrado["Unit price"].sum()
        return total_custo


    def calcular_margem_de_lucro_bruta(self, mes):
        total_do_mes = self.calcular_total_do_mes(mes)
        total_custo = self.calcular_total_custo(mes)
        margem_de_lucro_bruta = (total_do_mes - total_custo) / total_do_mes
