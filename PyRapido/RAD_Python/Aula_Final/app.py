...
ICONE_PATH = os.path.join(diretorio_atual, "icons8_user.ico")
janela.iconbitmap(ICONE_PATH)
janela.config(bg='#6666ff')
...
janela_incluir = None
diretorio_atual = os.path.dirname(os.path.abspath(__file__))
CAMINHO_DB = os.path.join(diretorio_atual, "contatos.sqlite")
...
# ---------------- FRAME PRINCIPAL -------------
topo = Frame(janela, width=500, bd=1, relief=SOLID)
topo.pack(side=TOP)
mid = Frame(janela, width=500, bg='#6666ff')
mid.pack(side=TOP)
...
arquivo_menu.add_command(label="Criar novo", command=inserir_dados)
arquivo_menu.add_separator()
arquivo_menu.add_command(label="Sair", command=janela.destroy)
